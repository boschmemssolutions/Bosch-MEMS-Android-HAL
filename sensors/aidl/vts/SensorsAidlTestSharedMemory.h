...
public:
  static SensorsAidlTestSharedMemory* create(ISensors::SharedMemInfo::SharedMemType type, size_t size) {
  ...
  ISensors::SharedMemInfo getSharedMemInfo() const {
    ISensors::SharedMemInfo mem = {.type = mType,
  ...
  std::vector<Event> parseEvents(int64_t lastCounter = -1, size_t offset = 0) const {
    constexpr size_t kEventSize = static_cast<size_t>(BnSensors::DIRECT_REPORT_SENSOR_EVENT_TOTAL_LENGTH);
    constexpr size_t kOffsetSize = static_cast<size_t>(BnSensors::DIRECT_REPORT_SENSOR_EVENT_OFFSET_SIZE_FIELD);
    constexpr size_t kOffsetToken = static_cast<size_t>(BnSensors::DIRECT_REPORT_SENSOR_EVENT_OFFSET_SIZE_REPORT_TOKEN);
    ...
    constexpr size_t kOffsetAtomicCounter =
      static_cast<size_t>(BnSensors::DIRECT_REPORT_SENSOR_EVENT_OFFSET_SIZE_ATOMIC_COUNTER);
    constexpr size_t kOffsetTimestamp =
      static_cast<size_t>(BnSensors::DIRECT_REPORT_SENSOR_EVENT_OFFSET_SIZE_TIMESTAMP);
    constexpr size_t kOffsetData = static_cast<size_t>(BnSensors::DIRECT_REPORT_SENSOR_EVENT_OFFSET_SIZE_DATA);
    ...
    while (offset + kEventSize <= mSize) {
      ...
      ALOGV("offset = %zu, cnt %" PRId64 ", token %" PRId32 ", type %" PRId32 ", timestamp %" PRId64, offset,
            atomicCounter, token, type, timestamp);
    ...
    switch (mType) {
      ...
      default: {
        if (mNativeHandle != nullptr || mSize != 0 || mBuffer != nullptr) {
          ALOGE(
            "SensorsAidlTestSharedMemory %p not properly destructed: "
  ...
  SensorsAidlTestSharedMemory(ISensors::SharedMemInfo::SharedMemType type, size_t size)
    ...
    switch (type) {
      case ISensors::SharedMemInfo::SharedMemType::ASHMEM: {
        ...
        if (handle != nullptr) {
          handle->data[0] = fd = ::ashmem_create_region("SensorsAidlTestSharedMemory", size);
          if (handle->data[0] > 0) {
            // memory is pinned by default
            buffer = static_cast<char*>(::mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0));
