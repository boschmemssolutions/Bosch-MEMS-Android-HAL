...
constexpr size_t kEventSize = static_cast<size_t>(ISensors::DIRECT_REPORT_SENSOR_EVENT_TOTAL_LENGTH);
...
bool isDirectReportRateSupported(SensorInfo sensor, ISensors::RateLevel rate) {
  unsigned int r = static_cast<unsigned int>(sensor.flags & SensorInfo::SENSOR_FLAG_BITS_MASK_DIRECT_REPORT) >>
...
void assertTypeMatchReportMode(SensorType type, int reportMode) {
  ...
  ASSERT_TRUE(expected == INT32_MAX || expected == reportMode)
    << "reportMode=" << static_cast<int>(reportMode) << "expected=" << static_cast<int>(expected);
}
...
void checkIsOk(ndk::ScopedAStatus status) { ASSERT_TRUE(status.isOk()); }
...
public:
  ...
  void waitForFlushEvents(const std::vector<SensorInfo>& sensorsToWaitFor, int32_t numCallsToFlush,
  ...
  void waitForEvents(const std::vector<SensorInfo>& sensorsToWaitFor, std::chrono::milliseconds timeout) {
    std::unique_lock<std::recursive_mutex> lock(mEventMutex);
...
protected:
  ...
  void verifyRegisterDirectChannel(std::shared_ptr<SensorsAidlTestSharedMemory<SensorType, Event>> mem,
  ...
  void queryDirectChannelSupport(ISensors::SharedMemInfo::SharedMemType memType, bool* supportsSharedMemType,
                                 bool* supportsAnyDirectChannel);

  void verifyUnregisterDirectChannel(int32_t* directChannelHandle, bool supportsAnyDirectChannel);

  void checkRateLevel(const SensorInfo& sensor, int32_t directChannelHandle, ISensors::RateLevel rateLevel,
                      int32_t* reportToken);
  ...
  ndk::ScopedAStatus batch(int32_t sensorHandle, int64_t samplingPeriodNs, int64_t maxReportLatencyNs) {
  ...
  ndk::ScopedAStatus registerDirectChannel(const ISensors::SharedMemInfo& mem, int32_t* aidlReturn);

  ndk::ScopedAStatus unregisterDirectChannel(int32_t* channelHandle) {
    return getSensors()->unregisterDirectChannel(*channelHandle);
  }

  ndk::ScopedAStatus configDirectReport(int32_t sensorHandle, int32_t channelHandle, ISensors::RateLevel rate,
                                        int32_t* reportToken) {
    return getSensors()->configDirectReport(sensorHandle, channelHandle, rate, reportToken);
  }

  void runSingleFlushTest(const std::vector<SensorInfo>& sensors, bool activateSensor, int32_t expectedFlushCount,
                          bool expectedResult);

  void runFlushTest(const std::vector<SensorInfo>& sensors, bool activateSensor, int32_t flushCalls,
                    int32_t expectedFlushCount, bool expectedResult);

  void checkVec3Sensor(SensorType type, const Event::EventPayload::Vec3& lowerLimit,
                       const Event::EventPayload::Vec3& upperLimit);

  inline static int32_t extractReportMode(int32_t flag) {
    return (flag & (SensorInfo::SENSOR_FLAG_BITS_CONTINUOUS_MODE | SensorInfo::SENSOR_FLAG_BITS_ON_CHANGE_MODE |
                    SensorInfo::SENSOR_FLAG_BITS_ONE_SHOT_MODE | SensorInfo::SENSOR_FLAG_BITS_SPECIAL_REPORTING_MODE));
  }
...
ndk::ScopedAStatus SensorsAidlTest::registerDirectChannel(const ISensors::SharedMemInfo& mem, int32_t* aidlReturn) {
...
void SensorsAidlTest::activateAllSensors(bool enable) {
  for (const SensorInfo& sensorInfo : getSensorsList()) {
    if (isValidType(sensorInfo.type)) {
      checkIsOk(batch(sensorInfo.sensorHandle, sensorInfo.minDelayUs, 0 /* maxReportLatencyNs */));
...
void SensorsAidlTest::runSingleFlushTest(const std::vector<SensorInfo>& sensors, bool activateSensor,
                                         int32_t expectedFlushCount, bool expectedResult) {
  runFlushTest(sensors, activateSensor, 1 /* flushCalls */, expectedFlushCount, expectedResult);
}

void SensorsAidlTest::runFlushTest(const std::vector<SensorInfo>& sensors, bool activateSensor, int32_t flushCalls,
                                   int32_t expectedFlushCount, bool expectedResult) {
...
TEST_P(SensorsAidlTest, SensorListValid) {
  ...
  for (size_t i = 0; i < sensorInfoList.size(); ++i) {
    ...
    SCOPED_TRACE(::testing::Message() << i << "/" << sensorInfoList.size() << ": "
                                      << " handle=0x" << std::hex << std::setw(8) << std::setfill('0')
    ...
    EXPECT_FALSE(info.type == SensorType::ADDITIONAL_INFO || info.type == SensorType::META_DATA);

    EXPECT_GE(info.fifoMaxEventCount, info.fifoReservedEventCount);

    // Test Reporting mode valid
    EXPECT_NO_FATAL_FAILURE(assertTypeMatchReportMode(info.type, extractReportMode(info.flags)));

    // Test min max are in the right order
    EXPECT_LE(info.minDelayUs, info.maxDelayUs);
    // Test min/max delay matches reporting mode
    EXPECT_NO_FATAL_FAILURE(
      assertDelayMatchReportMode(info.minDelayUs, info.maxDelayUs, extractReportMode(info.flags)));
  }
}

// Test if sensor list contains Bosch sensor
TEST_P(SensorsAidlTest, SensorListContainsBoschSensor) {
  std::vector<SensorInfo> sensorInfoList = getSensorsList();
...
TEST_P(SensorsAidlTest, SetOperationMode) {
  if (getInjectEventSensors().size() > 0) {
    ...
    int errorCode = getSensors()->setOperationMode(ISensors::OperationMode::DATA_INJECTION).getExceptionCode();
    ASSERT_TRUE((errorCode == EX_UNSUPPORTED_OPERATION) || (errorCode == EX_ILLEGAL_ARGUMENT));
  }
}
...
TEST_P(SensorsAidlTest, CallInitializeTwice) {
  ...
  public:
    SensorsAidlEnvironmentTest(const std::string& service_name) : SensorsAidlEnvironment(service_name) {}
  };
  ...
  std::unique_ptr<SensorsAidlEnvironmentTest> newEnv = std::make_unique<SensorsAidlEnvironmentTest>(GetParam());
  newEnv->SetUp();
  if (HasFatalFailure()) {
...
TEST_P(SensorsAidlTest, FlushSensor) {
  ...
  runSingleFlushTest(sensors, true /* activateSensor */, 1 /* expectedFlushCount */, true /* expectedResult */);
...
TEST_P(SensorsAidlTest, FlushOneShotSensor) {
  ...
  runSingleFlushTest(sensors, true /* activateSensor */, 0 /* expectedFlushCount */, false /* expectedResult */);
}
...
TEST_P(SensorsAidlTest, FlushInactiveSensor) {
  ...
  runSingleFlushTest(sensors, false /* activateSensor */, 0 /* expectedFlushCount */, false /* expectedResult */);
}
...
TEST_P(SensorsAidlTest, Batch) {
  ...
  for (const SensorInfo& sensor : getSensorsList()) {
    ...
    int64_t samplingPeriodNs =
      extractReportMode(sensor.flags) == SensorInfo::SENSOR_FLAG_BITS_ONE_SHOT_MODE ? 0 : sensor.minDelayUs;
  ...
  ASSERT_EQ(batch(sensor.sensorHandle, sensor.minDelayUs, 0 /* maxReportLatencyNs */).getExceptionCode(),
            EX_ILLEGAL_ARGUMENT);
}
...
void SensorsAidlTest::checkVec3Sensor(SensorType type, const Event::EventPayload::Vec3& lowerLimit,
                                      const Event::EventPayload::Vec3& upperLimit) {
  ...
  checkIsOk(activate(sensorInfo.sensorHandle, 1));
  ...
  const Event::EventPayload::Vec3& v = events[0].payload.get<Event::EventPayload::Tag::vec3>();
...
TEST_P(SensorsAidlTest, AccelerometerCheckSensorVector) {
  const Event::EventPayload::Vec3 lowerLimit = {-0.3, -0.3, GRAVITY_EARTH - 0.3, SensorStatus::ACCURACY_HIGH};
  const Event::EventPayload::Vec3 upperLimit = {0.3, 0.3, GRAVITY_EARTH + 0.3, SensorStatus::ACCURACY_HIGH};
  checkVec3Sensor(SensorType::ACCELEROMETER, lowerLimit, upperLimit);
}

// Test if gyroscope sensor vector is in range
TEST_P(SensorsAidlTest, GyroscopeCheckSensorVector) {
  const Event::EventPayload::Vec3 lowerLimit = {-0.1, -0.1, -0.1, SensorStatus::ACCURACY_HIGH};
  const Event::EventPayload::Vec3 upperLimit = {0.1, 0.1, 0.1, SensorStatus::ACCURACY_HIGH};
  checkVec3Sensor(SensorType::GYROSCOPE, lowerLimit, upperLimit);
}

// Test if gravity sensor vector is in range
TEST_P(SensorsAidlTest, GravityCheckSensorVector) {
  const Event::EventPayload::Vec3 lowerLimit = {-0.3, -0.3, GRAVITY_EARTH - 0.3, SensorStatus::ACCURACY_HIGH};
  const Event::EventPayload::Vec3 upperLimit = {0.3, 0.3, GRAVITY_EARTH + 0.3, SensorStatus::ACCURACY_HIGH};
  checkVec3Sensor(SensorType::GRAVITY, lowerLimit, upperLimit);
}

// Test if linear acceleration sensor vector is in range
TEST_P(SensorsAidlTest, LinearAccelerationCheckSensorVector) {
  const Event::EventPayload::Vec3 lowerLimit = {-0.3, -0.3, -0.3, SensorStatus::ACCURACY_HIGH};
  const Event::EventPayload::Vec3 upperLimit = {0.3, 0.3, 0.3, SensorStatus::ACCURACY_HIGH};
  checkVec3Sensor(SensorType::LINEAR_ACCELERATION, lowerLimit, upperLimit);
}

// Test if sensor name, vendor are as expected
TEST_P(SensorsAidlTest, ConfigCheck) {
...
TEST_P(SensorsAidlTest, NoStaleEvents) {
  ...
  for (const SensorInfo& sensor : sensors) {
    std::chrono::milliseconds minDelay =
      duration_cast<std::chrono::milliseconds>(std::chrono::microseconds(sensor.minDelayUs));
  ...
  for (const SensorInfo& sensor : sensors) {
    ...
    std::chrono::milliseconds delta = duration_cast<std::chrono::milliseconds>(
      std::chrono::nanoseconds(newEvent.timestamp - lastEventTimestampMap[sensor.sensorHandle]));
    std::chrono::milliseconds sensorMinDelay =
      duration_cast<std::chrono::milliseconds>(std::chrono::microseconds(sensor.minDelayUs));
...
void SensorsAidlTest::checkRateLevel(const SensorInfo& sensor, int32_t directChannelHandle,
  ...
  ndk::ScopedAStatus status = configDirectReport(sensor.sensorHandle, directChannelHandle, rateLevel, reportToken);
...
void SensorsAidlTest::queryDirectChannelSupport(ISensors::SharedMemInfo::SharedMemType memType,
  ...
  *supportsAnyDirectChannel = false;
  for (const SensorInfo& curSensor : getSensorsList()) {
    ...
    if (isDirectChannelTypeSupported(curSensor, ISensors::SharedMemInfo::SharedMemType::ASHMEM) ||
        isDirectChannelTypeSupported(curSensor, ISensors::SharedMemInfo::SharedMemType::GRALLOC)) {
...
void SensorsAidlTest::verifyRegisterDirectChannel(std::shared_ptr<SensorsAidlTestSharedMemory<SensorType, Event>> mem,
                                                  int32_t* directChannelHandle, bool supportsSharedMemType,
                                                  bool supportsAnyDirectChannel) {
...
void SensorsAidlTest::verifyUnregisterDirectChannel(int32_t* channelHandle, bool supportsAnyDirectChannel) {
...
void SensorsAidlTest::verifyConfigure(const SensorInfo& sensor, ISensors::SharedMemInfo::SharedMemType memType,
  ...
  if (isDirectChannelTypeSupported(sensor, memType)) {
    ...
    ndk::ScopedAStatus status =
      configDirectReport(-1 /* sensorHandle */, directChannelHandle, ISensors::RateLevel::NORMAL, &reportToken);
    ASSERT_EQ(status.getExceptionCode(), EX_ILLEGAL_ARGUMENT);

    status = configDirectReport(-1 /* sensorHandle */, directChannelHandle, ISensors::RateLevel::STOP, &reportToken);
    ASSERT_TRUE(status.isOk());
  } else {
    ...
    ndk::ScopedAStatus status =
      configDirectReport(sensor.sensorHandle, directChannelHandle, ISensors::RateLevel::NORMAL, &reportToken);
    ASSERT_EQ(status.getExceptionCode(), error);
  }
}

TEST_P(SensorsAidlTest, DirectChannelAshmem) { verifyDirectChannel(ISensors::SharedMemInfo::SharedMemType::ASHMEM); }

TEST_P(SensorsAidlTest, DirectChannelGralloc) { verifyDirectChannel(ISensors::SharedMemInfo::SharedMemType::GRALLOC); }
