...
static inline ::testing::AssertionResult isOk(const Return<T>& ret) {
  return (ret.isOk() ? ::testing::AssertionSuccess() : ::testing::AssertionFailure()) << ret.description();
}

template <>
__attribute__((__unused__)) inline ::testing::AssertionResult isOk(const Return<Result>& ret) {
  return ((ret.isOk() && ret == Result::OK) ? ::testing::AssertionSuccess() : ::testing::AssertionFailure())
         << ret.description() << ", " << (ret.isOk() ? toString(static_cast<Result>(ret)) : "");
}

static inline ::testing::AssertionResult isOk(Result result) {
  using ::android::frameworks::sensorservice::V1_0::toString;
  return (result == Result::OK ? ::testing::AssertionSuccess() : ::testing::AssertionFailure()) << toString(result);
}
  ...
  for (iter = begin, pos = 0; iter != end; ++iter, ++pos) {
    ...
    if (getField(*iter) < getField(*lastValue)) {
      return ::testing::AssertionFailure()
             << "Not an increasing sequence, pos = " << pos << ", " << getField(*iter) << " < " << getField(*lastValue);
    }
  }
...
public:
  ...
  android::base::Result<std::vector<SensorInfo>> GetSensorList(const std::function<bool(SensorInfo)>& pred = nullptr) {
...
TEST_P(SensorManagerTest, List) { ASSERT_RESULT_OK(GetSensorList()); }

TEST_P(SensorManagerTest, Ashmem) {
  auto ashmem_sensors =
    GetSensorList([](const auto& info) { return info.flags & SensorFlagBits::DIRECT_CHANNEL_ASHMEM; });
...
TEST_P(SensorManagerTest, Accelerometer) {
  ...
  auto accelerometer_ashmem_sensors = GetSensorList([](const auto& info) {
    ...
    int maxLevel = (info.flags & SensorFlagBits::MASK_DIRECT_REPORT) >> (int)SensorFlagShift::DIRECT_REPORT;
  ...
  for (const auto& info : *accelerometer_ashmem_sensors) {
    ...
    ASSERT_OK(ashmem_->allocate(memSize, [&](bool success, const auto& mem) {
      ...
      ASSERT_OK(manager_->createAshmemDirectChannel(mem, memSize, [&](const auto& chan, Result result) {
        ...
        EXPECT_TRUE(isIncreasing(events.begin(), events.end(), [](const auto& event) { return event.timestamp; }))
          << "timestamp is not monotonically increasing";
        for (const auto& event : events) {
          EXPECT_EQ(returnedToken, event.sensorHandle) << "configure token and sensor handle don't match.";
        }
      }));
    }));
  }
...
INSTANTIATE_TEST_SUITE_P(PerInstance, SensorManagerTest,
