...
public:
  ScopedAStatus onDynamicSensorsConnected(const std::vector<SensorInfo>& /* sensorInfos */) override {
    return ScopedAStatus::ok();
  }

  ScopedAStatus onDynamicSensorsDisconnected(const std::vector<int32_t>& /* sensorHandles */) override {
    return ScopedAStatus::ok();
  }
};
...
bool SensorsAidlEnvironment::resetHal() {
  bool succeed = false;
  do {
    mSensors = ISensors::fromBinder(SpAIBinder(AServiceManager_waitForService(mServiceName.c_str())));
    if (mSensors == nullptr) {
      break;
    }
    ...
    // Initialize FMQs
    mWakeLockQueue = std::make_unique<WakeLockQueue>(MAX_RECEIVE_BUFFER_EVENT_COUNT, true /* configureEventFlagWord */);
    mEventQueue = std::make_unique<EventQueue>(MAX_RECEIVE_BUFFER_EVENT_COUNT, true /* configureEventFlagWord */);
